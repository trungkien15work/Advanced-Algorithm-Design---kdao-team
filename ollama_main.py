    ...
    def __init__(self, data_config):
        ...
        try:
            ...
            for i in range(self.n_users):
                if i in augmented_user_init_embedding:
                    augmented_user_init_embedding_list.append(
                        augmented_user_init_embedding[i]
                    )
                else:
                    augmented_user_init_embedding_list.append(np.zeros(384))
        ...
        embed_file_path = args.data_path + args.dataset + "/augmented_total_embed_dict"

        if os.path.exists(embed_file_path):
            print(f"Loading existing item embeddings from {embed_file_path}")
            self.item_attribute_embedding = pickle.load(open(embed_file_path, "rb"))
        else:
            print("Generating aligned item embeddings (Safe Mode)...")

            if args.dataset == "preprocessed_raw_MovieLens":
                keys = ["title", "genre", "director", "country", "language"]
            else:
                keys = ["year", "title", "director", "country", "language"]

            augmented_total_embed_dict = {k: [] for k in keys}
            ...
            for value in keys:
                sample_dim = 384
                for k, v in augmented_atttribute_embedding_dict.get(value, {}).items():
                    sample_dim = len(v)
                    break
                ...
                for i in range(self.n_items):
                    if (
                        value in augmented_atttribute_embedding_dict
                        and i in augmented_atttribute_embedding_dict[value]
                    ):
                        temp_list.append(augmented_atttribute_embedding_dict[value][i])
                    else:
                        temp_list.append(np.zeros(sample_dim))

                augmented_total_embed_dict[value] = np.array(temp_list)

            pickle.dump(augmented_total_embed_dict, open(embed_file_path, "wb"))
        ...
        for epoch in range(args.epoch):
            ...
            for idx in tqdm(range(n_batch)):
                ...
                augmented_sample_dict = pickle.load(
                    open(
                        args.data_path + args.dataset + "/augmented_sample_dict",
                        "rb",
                    )
                )
                users_aug = random.sample(users, int(len(users) * args.aug_sample_rate))

                # Safe access for augmented samples
                pos_items_aug = []
                neg_items_aug = []
                valid_users_aug = []
                ...
                for user in users_aug:
                    if user in augmented_sample_dict:
                        p_item = augmented_sample_dict[user][0]
                        n_item = augmented_sample_dict[user][1]
                        if p_item < self.n_items and n_item < self.n_items:
                            pos_items_aug.append(p_item)
                            neg_items_aug.append(n_item)
                            valid_users_aug.append(user)

                self.new_batch_size = len(valid_users_aug)
                users += valid_users_aug
