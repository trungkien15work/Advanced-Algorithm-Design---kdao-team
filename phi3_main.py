    ...
    def train(self):
        ...
        for epoch in range(args.epoch):
            ...
            for idx in tqdm(range(n_batch)):
                ...
                users_aug_potential = random.sample(users, int(len(users)*args.aug_sample_rate))
                valid_aug_users = []
                for user in users_aug_potential:
                    if user in augmented_sample_dict:
                        pos_id = augmented_sample_dict[user][0]
                        neg_id = augmented_sample_dict[user][1]
                        if pos_id < self.n_items and neg_id < self.n_items:
                            valid_aug_users.append(user)

                users_aug = valid_aug_users
                pos_items_aug = [augmented_sample_dict[user][0] for user in users_aug]
                neg_items_aug = [augmented_sample_dict[user][1] for user in users_aug]
